...
The more advanced user can get along with an Debian installation disk 
only. The installer disk has an excellent "Rescue mode" that automates many of 
the steps below.

1. Do not boot a live-syste, but boot your legacy Debian system as usual.
...
. Create a new 43MB partition for efi using `gparted` with partition code `EF00`
